    ...
    protected function renderContent() {
        if (Yii::app()->params['showBanner']) {
            if (isset($_GET['show_banner_position'])) {
                ...
                if ($this->position === 4 or $this->position ===8  or $this->position ===6  or $this->position ===2   or $this->position ===3 or $this->position ===1 or $this->position ===102) {
                    $items = Banner::model()->findAll('`position` = ' . $this->position . ' AND `published` = 1 AND `end` >=NOW()');
                    $count = count($items);
                    if ($count > 1) {
                        $rand = rand(1, $count);
                        $id = array();
