...
class Poll extends CActiveRecord {
    ...
    public function userCanVote() {
        ...
        #echo "key: " . $key;
        if ($key) {
            $bkey = PollVote::model()->find('`key` = "' . $key . '" and poll_id = ' . $this->id);
            if (count($bkey) > 0)
                return false;
        }
        ...
        $where = array('and', 'poll_id=:poll_id', 'user_id=:user_id', '`key`=:key');
        $params = array(':poll_id' => $this->id, ':user_id' => (int) Yii::app()->user->id, ':key' => $key);
    ...
    public function getCountvotes() {
        $result = Yii::app()->db->createCommand(array(
                    'select' => 'count(*) as c',
                    'from' => '{{poll_vote}}',
                    'where' => array('poll_id=:pollid'),
                    'params' => array(':pollid' => $this->id),
                ))->queryRow();
        return $result['c'];
    }
