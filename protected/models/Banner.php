...
class Banner extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('position, start, client_id', 'required'),
			array('position, client_id, published, width, height', 'numerical', 'integerOnly'=>true),
			array('img, link', 'length', 'max'=>255),
			array('key', 'length', 'max'=>32),
			array('end, code', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, position, start, end, img, code, link, client_id, published, key, width, height', 'safe', 'on'=>'search'),
		);
	}
